...
md`# Story timeline visualization (Hunger Games Trilogy)`
)}
...
function _chapterData(FileAttachment){return(
FileAttachment("hunger_games_3_books_chapter_sessions_enriched.json").json()
)}

function _storyData(FileAttachment){return(
FileAttachment("hunger_games_in_story_timeline_complete_renumbered.json").json()
)}

function _data(timelineMode,storyData,chapterData){return(
timelineMode === "story" ? storyData : chapterData
)}
...
function _chapterSceneMetaByKey(chapterData)
{
  const m = new Map();
...
function _storySceneMetaByKey(storyData)
{
  const m = new Map();
...
function _sceneMeta(timelineMode,storySceneMetaByKey,chapterSceneMetaByKey,emptySceneMeta){return(
(globalChapter, sceneId) => {
  const key = `${globalChapter}|${sceneId}`;

  const primary =
    timelineMode === "story"
      ? storySceneMetaByKey.get(key)
      : chapterSceneMetaByKey.get(key);
...
({ globalChapter, sceneId, focusCharacter = null } = {}) => {
...
new Map([
  ["district12-seam", "#E69F00"],
  ["district12-family", "#009E73"],
  ["district12-victors", "#CC79A7"],
  ["district12-survivor", "#B07AA1"],
  ["tribute", "#F0E442"],
  ["tribute-d12", "#D55E00"],
  ["career-pack", "#DC322F"],
  ["district11-allies", "#56B4E9"],
  ["quarter-quell-alliance", "#7E57C2"],
  ["capitol-control", "#0072B2"],
  ["capitol-media", "#17A398"],
  ["capitol-functionary", "#8C564B"],
  ["capitol-stylist", "#FF66B3"],
  ["district12-merchants", "#8BC34A"],
  ["district13-rebels", "#FF6F61"],
  ["star-squad", "#9E9E9E"]
])
)}
...
new Map([
  // District 12 / victor labels
  ["victor-d12", "district12-victors"],
  ["district12-victor", "district12-victors"],
  ["district12-survivor", "district12-survivor"],

  // Quarter Quell / arena alliance labels
  ["quarter-quell-tribute", "quarter-quell-alliance"],
  ["district4-victor", "quarter-quell-alliance"],
  ["arena-survivor", "quarter-quell-alliance"],
  ["victor-network", "quarter-quell-alliance"],
  ["covert-rebel", "quarter-quell-alliance"],

  // District 13 / rebel-side labels
  ["rebel-strategist", "district13-rebels"],
  ["rebel-leadership", "district13-rebels"],
  ["district13-command", "district13-rebels"],
  ["victor-ally", "district13-rebels"],
  ["mockingjay-symbol", "district13-rebels"],
  ["rebel-soldier", "district13-rebels"],

  // Capitol-side labels
  ["former-capitol-insider", "capitol-functionary"],
  ["mentor", "capitol-functionary"],
  ["prep-team", "capitol-stylist"],

  // Star Squad variants
  ["star-squad-451", "star-squad"]
])
)}
...
(globalChapter, sceneId) => {
  const edges = sceneMeta(globalChapter, sceneId).relationship_edges ?? [];
...
function _maxStoryUnit(storyOrder){return(
storyOrder.chapterByRank.size
)}

function _spoilerMaxUnit(spoilerMaxChapter,storyOrder){return(
spoilerMaxChapter === Infinity
    ? Infinity
    : (storyOrder.rankByChapter.get(spoilerMaxChapter) ?? spoilerMaxChapter)
)}

function _maxAllowedUnit(timelineMode,spoilerMaxUnit,maxStoryUnit,spoilerMaxChapter,maxChapter){return(
timelineMode === "story"
    ? (spoilerMaxUnit === Infinity ? maxStoryUnit : Math.min(maxStoryUnit, spoilerMaxUnit))
    : (spoilerMaxChapter === Infinity ? maxChapter : Math.min(maxChapter, spoilerMaxChapter))
)}
...
function _povSceneStepEffect(currentChapter,povEnabled,$0,$1,Event)
{
  const _chapter = currentChapter; // rerun after chapter changes
  ...
  const targetStep = $0.value;
  if (targetStep == null) return null;

  const slider = $1;
  if (+slider.value !== +targetStep) {
...
function _debugPovButtons(Inputs,povCharacter,currentChapter,sceneStep,prevPovStop,nextPovStop){return(
Inputs.table([{
  povCharacter,
  currentChapter,
  sceneStep,
  prevGlobalChapter: prevPovStop?.globalChapter ?? null,
  prevChapterStep: prevPovStop?.chapterStep ?? null,
  nextGlobalChapter: nextPovStop?.globalChapter ?? null,
  nextChapterStep: nextPovStop?.chapterStep ?? null
}])
)}
...
function _storyJumpOptions(d3,storyData,formatStoryDateLabel)
{
  // one entry per story-date chapter (date index)
  const firstRow = d3.rollup(storyData, v => v[0], d => d.chapter);
  return Array.from(firstRow, ([ch, row]) => {
    const dateKey = row.date_key || row.story_time_iso || row.beat_short;
    const dateLabel = formatStoryDateLabel(dateKey);
    const detail = (row.beat_label || row.session_title || "").trim();
    const label = detail ? `${dateLabel} — ${detail}` : dateLabel;
    return [label, ch];
  }).sort((a,b) => a[1] - b[1]);
}


function _jumpDate(Inputs,storyJumpOptions,currentChapter){return(
Inputs.select(new Map(storyJumpOptions), {
  label: "Jump to date",
  value: currentChapter
})
)}

function _67(timelineMode,jumpDate,currentChapter,$0,Event)
{
  if (timelineMode === "story" && jumpDate != null && jumpDate !== currentChapter) {
    const el = $0;
    el.value = jumpDate;
...
function _finished(Inputs){return(
Inputs.toggle(false, {
  label: "I have finished the books (unlock in-story chronology & disable spoiler protection)"
})
)}

function _timelineWanted(Inputs){return(
Inputs.radio(["chapter", "story"], {
  label: "Timeline",
  value: "chapter",
  format: (v) => (v === "chapter" ? "By chapters / beats" : "In-story chronology")
})
)}

function _timelineMode(finished,timelineWanted){return(
finished ? timelineWanted : "chapter"
)}

function _books(chapterData,d3){return(
Array.from(new Set(chapterData.map(d => d.book))).sort(d3.ascending)
)}

function _beatsByBook(d3,chapterData){return(
d3.rollup(
  chapterData,
  v => {
    const rows = Array.from(
      d3.group(v, d => d.chapter),
      ([chapter, arr]) => ({
        chapter,
        beat_short: arr[0].beat_short,
        beat_label: arr[0].beat_label
      })
...
function _lastReadBook(Inputs,books,chapterData){return(
Inputs.select(books, {
  label: "Last read book",
  format: b => {
    const any = chapterData.find(d => d.book === b);
    return any?.book_name ?? `Book ${b}`;
  }
})
)}

function _chaptersByBook(d3,chapterData)
{
  // Map(book_name -> Array of unique chapters in that book)
  const m = d3.rollup(
    chapterData,
    (v) => ({
      chapter: v[0].chapter,                 // global chapter id
      chapter_in_book: v[0].chapter_in_book, // 1..27
      book_name: v[0].book_name,
      beat_label: v[0].beat_label            // use as chapter name
    }),
    (d) => d.book_name,
    (d) => d.chapter
  );

  // Convert Map(book -> Map(chapter -> meta)) to Map(book -> sorted array)
  return new Map(
    Array.from(m, ([book, byChapter]) => [
      book,
      Array.from(byChapter.values()).sort(
        (a, b) => d3.ascending(a.chapter_in_book, b.chapter_in_book)
      )
    ])
  );
}
...
function _chapterJumpOptions(maxChapter,spoilerMaxChapter,chaptersByBook,d3)
{
  const maxAllowed = Math.min(maxChapter, spoilerMaxChapter);
  const bookOrder = ["The Hunger Games", "Catching Fire", "Mockingjay"];

  const out = [];
  for (const book of bookOrder) {
    const rows = (chaptersByBook.get(book) || []).filter(d => d.chapter <= maxAllowed);
    out.push(...rows);
  }

  // Fallback if book names differ (still returns a sorted list)
  if (!out.length) {
    out.push(
      ...Array.from(chaptersByBook.values()).flat().filter(d => d.chapter <= maxAllowed)
    );
    out.sort((a, b) => d3.ascending(a.chapter, b.chapter));
  }
...
Inputs.select(chapterJumpOptions, {
  ...
  format: (d) => {
    ...
    const left = `${d.book_name} · Ch ${chNo}`;
...
function _91(Inputs,povStopsAll){return(
Inputs.table(povStopsAll.slice(0, 20))
)}
...
function _lastReadBeat(Inputs,chaptersByBook,lastReadBook){return(
Inputs.select(chaptersByBook.get(lastReadBook) ?? [], {
  label: "Last read chapter",
  format: d => `${d.book_name} · Chapter ${d.chapter_in_book}`
})
)}

function _spoilerMaxChapter(finished,lastReadBeat){return(
finished ? Infinity : (lastReadBeat?.chapter ?? 1)
)}

function _storyOrder(storyData,d3)
{
  const sorted = Array.from(new Set(storyData.map(d => d.chapter)))
    .sort((a,b) => d3.ascending(a, b));

  const rankByChapter = new Map();
  const chapterByRank = new Map();
  sorted.forEach((chapter, i) => {
    rankByChapter.set(chapter, i + 1);
    chapterByRank.set(i + 1, chapter);
  });

  return { rankByChapter, chapterByRank };
}


function _debugNavState(Inputs,timelineMode,currentChapter,jumpDate,jumpToChapter,maxAllowedUnit,spoilerMaxChapter,$0){return(
Inputs.table([{
  timelineMode,
  currentChapter,
  jumpDate: jumpDate ?? null,
  jumpToChapter: jumpToChapter?.chapter ?? null,
  maxAllowedUnit,
  spoilerMaxChapter,
  currentUnit: $0.value,
  currentUnitMatches: +currentChapter === +$0.value
}])
)}

function _chart(d3,width,height,storyOrder,data,leftMargin,bottomMargin,topMargin,rightMargin,motivationBySceneChar,windowSize,charactersList,yScale,transitionMs,formatStoryDateLabel,sceneMaxChars,colorForSceneChar,sceneBend,dominantSessionRelationshipFamily,relationshipStroke,relationshipStyle,$0,buildSceneDetail)
{
  // ----------------------------
  ...
  const chapterRefText = (m) => {
    if (!m) return "?";
    return m.chapter_in_book != null
      ? `Ch ${m.chapter_in_book}`
      : m.book_chapter_ref ?? "?";
  };
  ...
  svg
    ...
    .attr("fill", "#fff")
    .attr("font-size", 16)
    .text("Storyline Visualization (Hunger Games – prototype)");
  ...
  const BAND_MIN_W = 36; // minimum visible width (wider = easier to see)
  ...
  function computeDynamicY(scenesRaw, chaptersInWindow) {
    ...
    // final y positions: chapter|character -> y
    const yByKey = new Map();

    // session index: sessionKey -> { key,gid,chapter, chars:[...], y0,y1 }
    const sessionsIndex = new Map();

    const avail = height - bottomMargin - topMargin;

    // ======= spacing knobs =======
    const INTRA_STEP_PX = 38; // increase to spread lines within a session
    const MIN_INTRA_STEP_PX = 14; // floor so it never collapses too much
    const INTER_SESSION_GAP_PX = 34; // extra gap between sessions (visual grouping)
    // ==========================================
    ...
    for (const ch of chaptersInWindow) {
      ...
      // group into sessions like before
      const groups = d3.groups(
        scenesInCh,
        (d) => d.session_group ?? d.concurrency_group ?? d.scene_id
      );

      // build sessions and their char sets
      const sessions = groups.map(([gid, rows]) => {
        const chars = Array.from(
          new Set(rows.flatMap((r) => r.characters || []))
        ).filter((c) => yScale(c) != null);
        ...
        // sort chars by previous y so it stays smooth across chapters
        chars.sort(
          (a, b) => (prevY.get(a) ?? yScale(a)) - (prevY.get(b) ?? yScale(b))
        );
      ...
      // session ordering (same idea as yours)
      sessions.sort((a, b) => a.order - b.order);
      sessions.sort((a, b) => d3.ascending(a.bary, b.bary));

      // store session membership now (y extents computed later)
      ...
      // ----------------------------
      // key change: build a ONE-TIME chapter ordering of UNIQUE chars
      // ----------------------------
      const segments = [];
      const seen = new Set();
      ...
      // if no characters in this chapter, skip
      const totalChars = d3.sum(segments, (seg) => seg.length);
      if (!totalChars) continue;

      const totalGaps = Math.max(0, segments.length - 1);

      // compute how much vertical space we want to use
      // base used = between-char steps + between-session extra gaps
      const baseUsed =
        (totalChars - 1) * INTRA_STEP_PX + totalGaps * INTER_SESSION_GAP_PX;

      // if it doesn't fit, scale down steps (but keep min)
      ...
      // ----------------------------
      // compute final band extents AFTER y’s are finalized
      // (this fixes dots outside the band)
      // ----------------------------
      for (const s of sessions) {
        const ys = s.chars
  ...
  function update(state, { duration = transitionMs } = {}) {
    ...
    const idxBySceneKey = new Map(orderedForX.map((s, i) => [sceneKey(s), i]));
    const n = orderedForX.length;
    ...
    // span per session using ALL scenes in the window (orderedForX), not just revealed
    const sessionXRangeAll = new Map(); // key `${chapter}|${gid}` -> {x0,x1}
    ...
    for (const s of orderedForX) {
      ...
      if (!cur) sessionXRangeAll.set(key, { x0: x, x1: x });
      else
    ...
    const chaptersInX = Array.from(
      new Set(orderedForX.map((d) => d.chapter))
    ).sort((a, b) => a - b);

    const chapterSpan = new Map(); // ch -> {i0,i1,centerTick}
    for (const ch of chaptersInX) {
      const idxs = orderedForX
      ...
      //chapterSpan.set(ch, { i0, i1, center });
      const start = i0; // tick at first scene
      const center = (i0 + i1) / 2; // keep if you still want it for other things
    ...
    const dots = scenesWithLayout.flatMap((s) => {
      ...
      const sessionKey = `${s.chapter}|${gid}`;
      return (s.characters || [])
        .filter((c) => yScale(c) != null) // only characters that are in the y layout
        .map((c) => ({
          key: `${c}|${s.chapter}|${s.scene_id}`,
          character: c,
          chapter: s.chapter,
          scene_id: s.scene_id,
          sessionKey,
    ...
    const bands = Array.from(sessionsIndex.values()).map((s) => {
      ...
      const x0 = xr.x0,
        x1 = xr.x1;
      const w = Math.max(BAND_MIN_W, x1 - x0 + 2 * BAND_PAD_X);
      const x = (x0 + x1) / 2 - w / 2;
      //const y = (s.y0 ?? 0) - BAND_PAD_Y;
...
function _currentChapterLabel(timelineMode,d3,chapterData,currentChapter,storyData,formatStoryDateLabel)
{
  if (timelineMode !== "story") {
    const byChapter = d3.rollup(chapterData, v => v[0], d => d.chapter);
    const m = byChapter.get(currentChapter);
    if (!m) return `Chapter ${currentChapter}`;
    const left = `${m.book_name ?? m.book} · Ch ${String(m.chapter_in_book ?? "?").padStart(2, "0")}`;
    const right = (m.beat_label || "").trim();
    return right ? `${left} — ${right}` : left;
  }

  const storyRow = d3.rollup(storyData, v => v[0], d => d.chapter).get(currentChapter);
  if (!storyRow) return `Story position ${currentChapter}`;

  const exact =
    storyRow.date_key ||
    storyRow.story_time_iso ||
    storyRow.story_date ||
    storyRow.beat_short ||
    "";

  const exactLabel = formatStoryDateLabel(exact) || exact || `T${currentChapter}`;
  const detail = (storyRow.session_title || storyRow.beat_label || "").trim();

  return detail ? `${exactLabel} — ${detail}` : exactLabel;
}
...
function _controlsBar(html,finished,$0,$1,$2,$3,timelineMode,$4,$5,$6,$7,$8,makePovNavButton,prevPovStop,nextPovStop){return(
html`
<style>
  .sb889-wrap{
    width: 100%;
  ...
  .sb889-navbtn:hover:not(:disabled){
    background: #3f4d63;
    border-color: #64748b;
    box-shadow: 0 2px 6px rgba(15, 23, 42, 0.22);
  }

  .sb889-navbtn:active:not(:disabled){
    background: #273244;
    transform: translateY(1px);
  }

  .sb889-navbtn:focus-visible{
    outline: 2px solid #93c5fd;
    outline-offset: 2px;
  }

  .sb889-navbtn:disabled{
    background: #cbd5e1;
    color: #6b7280;
    border-color: #cbd5e1;
    opacity: 1;
  ...
  .sb889-card select,
  .sb889-card input[type="number"],
  .sb889-card input[type="text"],
  .sb889-card input[type="range"]{
...
<div class="sb889-wrap">
  ...
  <div class="${finished ? "sb889-topgrid-single" : "sb889-topgrid"}">
        ...
        finished
          ? html`
              <div class="sb889-help">
                You marked “finished”, so spoiler protection is disabled and in-story chronology is available.
              </div>
              <div style="margin-top:8px;" class="sb889-short">
                ${$1}
              </div>
            `
          : html`
              <div class="sb889-mode">Timeline: By Chapters (spoiler-safe)</div>
              <div class="sb889-notice">
                In-story timeline is locked until you mark “finished”.
              </div>
            `
      }
    </div>
    ...
    ${
      finished
        ...
        : html`
            <div class="sb889-card">
              <h4>Last read position</h4>
              <div class="sb889-stack">
                <div class="sb889-short">${$2}</div>
                <div class="sb889-short">${$3}</div>
              </div>
              <div class="sb889-help">You can only navigate up to this point.</div>
            </div>
          `
    }
  </div>
  ...
  <div class="sb889-bottomgrid">
    <div class="sb889-card">
      ...
      ${
        timelineMode === "story"
          ? html`
              <div class="sb889-short">${$4}</div>
              <div style="margin-top:6px;" class="sb889-short">
                ${$5}
              </div>
              <div class="sb889-help">
                Jump within the in-story date index.
              </div>
            `
          : html`
              <div class="sb889-short">${$6}</div>
              <div style="margin-top:6px;" class="sb889-short">
                ${$5}
              </div>
              <div class="sb889-help">
                Jump within the allowed chapter range.
              </div>
            `
      }
    </div>
    ...
    ${
      timelineMode === "story"
        ? html`<div class="sb889-card">
            <h4>POV navigation</h4>
            <div class="sb889-help">
              POV navigation is available only in chapter mode.
            </div>
          </div>`
        : html`<div class="sb889-card">
            <h4>POV navigation</h4>

            <div class="sb889-short">${$7}</div>
            ...
            <div class="sb889-btnrow">
              ${makePovNavButton("Previous appearance", prevPovStop)}
              ${makePovNavButton("Next appearance", nextPovStop)}
            </div>

            <div class="sb889-help">
              Jump to a tagged POV scene, or move to the previous/next appearance of the selected character.
            </div>
          </div>`
    }
  </div>
...
<details class="sb889-instructions" open>
  ...
  <h5>Reading mode and spoiler protection</h5>
  <ul>
    <li>Leave <span class="sb889-kbd">Finished reading?</span> unchecked to stay in spoiler-safe mode.</li>
    <li>In spoiler-safe mode, the chart stays in <strong>By chapters / beats</strong> and limits navigation to your <strong>Last read position</strong>.</li>
    <li>After checking <span class="sb889-kbd">Finished reading?</span>, you can switch to <strong>In-story chronology</strong> and move freely through the full story.</li>
  </ul>

  <h5>Navigation</h5>
  <ul>
    <li>Use <span class="sb889-kbd">Jump to chapter</span> to move directly to a chapter in reading order.</li>
    <li>Use the <span class="sb889-kbd">Chapter</span> slider to step through the allowed range one chapter at a time.</li>
    <li>In in-story mode, use <span class="sb889-kbd">Jump to date</span> and the in-story position slider to move through the story chronology instead of reading order.</li>
  </ul>

  <h5>POV navigation</h5>
  <ul>
    <li>Choose a character in <span class="sb889-kbd">POV character</span> to follow their appearances through the story.</li>
    <li>Use <span class="sb889-kbd">Jump to POV scene</span> to jump directly to a tagged scene for that character.</li>
    <li>Use <span class="sb889-kbd">Previous appearance</span> and <span class="sb889-kbd">Next appearance</span> to move between nearby appearances of that selected character.</li>
  </ul>

  <h5>How to read the chart</h5>
  <ul>
    <li><strong>Colored lines</strong> represent characters over time.</li>
    <li><strong>Rounded vertical bands</strong> represent interaction sessions.</li>
    <li>If multiple bands line up at the same x-position, they represent <strong>concurrent interactions</strong> happening at the same story moment.</li>
    <li>The <strong>Legend</strong> explains group/faction colors and relationship line styles.</li>
  </ul>

  <h5>Interaction</h5>
  <ul>
    <li>Click a <strong>band</strong> to inspect that interaction session.</li>
    <li>Click a <strong>line</strong> to show that character’s <strong>last visible appearance</strong> in the side panel and hover panel.</li>
    <li>Click a <strong>point</strong> to show that character’s <strong>scene-specific description</strong> with respect to that interaction session.</li>
    <li>The <strong>Scene details</strong> panel below the chart updates after click and stays visible.</li>
  </ul>

  <h5>What this prototype supports</h5>
  <p>
    This prototype is designed to support spoiler-safe reading, character-following, and comparison between chapter order and in-story chronology.
  </p>
</div>
</details>
`
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["hunger_games_3_books_chapter_sessions_enriched.json", {url: new URL("./files/68cf98dd05b0dc320370386651d21b0683628406bf79f7e615a20e362b7d8a32816d3e5307f017ebc19c09062c0b084194b8fa585cd2c8d40127264100525ae7.json", import.meta.url), mimeType: "application/json", toString}],
    ["hunger_games_in_story_timeline_complete_renumbered.json", {url: new URL("./files/5776d65996ec362a4fd831476d9aa56471ddafb3b15e66386b0ee532b5fdaf13b07a9b79d82a13483fc6ba9dc1349a61544a6d7e86f3c3f72cc9add3a7dab363.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("d3")).define("d3", ["require"], _d3);
  main.variable(observer("chapterData")).define("chapterData", ["FileAttachment"], _chapterData);
  main.variable(observer("storyData")).define("storyData", ["FileAttachment"], _storyData);
  main.variable(observer("data")).define("data", ["timelineMode","storyData","chapterData"], _data);
  ...
  main.variable(observer("chapterSceneMetaByKey")).define("chapterSceneMetaByKey", ["chapterData"], _chapterSceneMetaByKey);
  main.variable(observer("storySceneMetaByKey")).define("storySceneMetaByKey", ["storyData"], _storySceneMetaByKey);
  main.variable(observer("emptySceneMeta")).define("emptySceneMeta", _emptySceneMeta);
  main.variable(observer("sceneMeta")).define("sceneMeta", ["timelineMode","storySceneMetaByKey","chapterSceneMetaByKey","emptySceneMeta"], _sceneMeta);
  ...
  main.variable(observer("maxStoryUnit")).define("maxStoryUnit", ["storyOrder"], _maxStoryUnit);
  main.variable(observer("spoilerMaxUnit")).define("spoilerMaxUnit", ["spoilerMaxChapter","storyOrder"], _spoilerMaxUnit);
  main.variable(observer("maxAllowedUnit")).define("maxAllowedUnit", ["timelineMode","spoilerMaxUnit","maxStoryUnit","spoilerMaxChapter","maxChapter"], _maxAllowedUnit);
  ...
  main.variable(observer("povSceneStepEffect")).define("povSceneStepEffect", ["currentChapter","povEnabled","mutable pendingPovStep","viewof sceneStep","Event"], _povSceneStepEffect);
  main.variable(observer("debugPovButtons")).define("debugPovButtons", ["Inputs","povCharacter","currentChapter","sceneStep","prevPovStop","nextPovStop"], _debugPovButtons);
  ...
  main.variable(observer("storyJumpOptions")).define("storyJumpOptions", ["d3","storyData","formatStoryDateLabel"], _storyJumpOptions);
  main.variable(observer("viewof jumpDate")).define("viewof jumpDate", ["Inputs","storyJumpOptions","currentChapter"], _jumpDate);
  main.variable(observer("jumpDate")).define("jumpDate", ["Generators", "viewof jumpDate"], (G, _) => G.input(_));
  main.variable(observer()).define(["timelineMode","jumpDate","currentChapter","viewof currentChapter","Event"], _67);
  main.variable(observer("viewof finished")).define("viewof finished", ["Inputs"], _finished);
  main.variable(observer("finished")).define("finished", ["Generators", "viewof finished"], (G, _) => G.input(_));
  main.variable(observer("viewof timelineWanted")).define("viewof timelineWanted", ["Inputs"], _timelineWanted);
  main.variable(observer("timelineWanted")).define("timelineWanted", ["Generators", "viewof timelineWanted"], (G, _) => G.input(_));
  main.variable(observer("timelineMode")).define("timelineMode", ["finished","timelineWanted"], _timelineMode);
  main.variable(observer("books")).define("books", ["chapterData","d3"], _books);
  main.variable(observer("beatsByBook")).define("beatsByBook", ["d3","chapterData"], _beatsByBook);
  main.variable(observer("viewof lastReadBook")).define("viewof lastReadBook", ["Inputs","books","chapterData"], _lastReadBook);
  main.variable(observer("lastReadBook")).define("lastReadBook", ["Generators", "viewof lastReadBook"], (G, _) => G.input(_));
  main.variable(observer("chaptersByBook")).define("chaptersByBook", ["d3","chapterData"], _chaptersByBook);
  main.variable(observer("chapterJumpOptions")).define("chapterJumpOptions", ["maxChapter","spoilerMaxChapter","chaptersByBook","d3"], _chapterJumpOptions);
  ...
  main.variable(observer()).define(["Inputs","povStopsAll"], _91);
  ...
  main.variable(observer("viewof lastReadBeat")).define("viewof lastReadBeat", ["Inputs","chaptersByBook","lastReadBook"], _lastReadBeat);
  main.variable(observer("lastReadBeat")).define("lastReadBeat", ["Generators", "viewof lastReadBeat"], (G, _) => G.input(_));
  main.variable(observer("spoilerMaxChapter")).define("spoilerMaxChapter", ["finished","lastReadBeat"], _spoilerMaxChapter);
  main.variable(observer("storyOrder")).define("storyOrder", ["storyData","d3"], _storyOrder);
  main.variable(observer("debugNavState")).define("debugNavState", ["Inputs","timelineMode","currentChapter","jumpDate","jumpToChapter","maxAllowedUnit","spoilerMaxChapter","mutable currentUnit"], _debugNavState);
  main.variable(observer("chart")).define("chart", ["d3","width","height","storyOrder","data","leftMargin","bottomMargin","topMargin","rightMargin","motivationBySceneChar","windowSize","charactersList","yScale","transitionMs","formatStoryDateLabel","sceneMaxChars","colorForSceneChar","sceneBend","dominantSessionRelationshipFamily","relationshipStroke","relationshipStyle","mutable selectedSceneDetail","buildSceneDetail"], _chart);
  ...
  main.variable(observer("currentChapterLabel")).define("currentChapterLabel", ["timelineMode","d3","chapterData","currentChapter","storyData","formatStoryDateLabel"], _currentChapterLabel);
  ...
  main.variable(observer("controlsBar")).define("controlsBar", ["html","finished","viewof finished","viewof timelineWanted","viewof lastReadBook","viewof lastReadBeat","timelineMode","viewof jumpDate","viewof currentChapter","viewof jumpToChapter","viewof povCharacter","viewof jumpToPov","makePovNavButton","prevPovStop","nextPovStop"], _controlsBar);
